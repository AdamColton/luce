...
//type unsafeFieldGenerator[Ctx any] func(ptr unsafe.Pointer, ctx *MarshalContext[Ctx]) (string, WriteNode)
...
func GeneratedField[On, T, Ctx any](fg FieldGenerator[On, T, Ctx], ctx *TypesContext[Ctx]) {
	...
	if ot.Kind() == reflect.Pointer {
		ctx.fieldGenerators[ot.Elem()] = append(ctx.fieldGenerators[ot], out)
	} else {
		ctx.fieldGenerators[ot] = append(ctx.fieldGenerators[ot], out)
	}
