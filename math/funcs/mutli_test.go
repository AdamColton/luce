...
func TestSanityCheck(t *testing.T) {
	...
	check := func(fn, dfn func(t float64) (x, y float64)) {
		for i := range steps.Iter().Channel(0) {
			x, y := fn(i)
			dx, dy := dfn(i)
			x0, y0 := fn(i + d)
	...
	grid := numiter.Grid(0, 1, 1/resolution, 0, 1, 1/resolution)
	for pt := range grid.Iter().Channel(0) {
		y := dist([]float64{pt[0], pt[1]})
		yA := dist([]float64{pt[0] + d, pt[1]})
		yB := dist([]float64{pt[0], pt[1] + d})
		dya, dyb := ddist(pt[0], pt[1])
