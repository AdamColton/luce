...
// Seq applys the TransformFunc to an iter.Seq
func (fn TransformFunc[In, Out]) Seq(inSeq iter.Seq[In]) iter.Seq[Out] {
	idx := 0
	return func(yield func(out Out) bool) {
		inSeq(func(in In) bool {
			out, include := fn(in, idx)
			idx++
			if include {
				return yield(out)
			}
