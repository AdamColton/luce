...
func (i *idxCheck) HandleIter(itr Iterator) {
...
func TestMRI(t *testing.T) {
	...
	for n, tc := range tt {
		t.Run(n, func(t *testing.T) {
			mri := tc.Match.Root("/")
			got := slice.FromIterFactory(mri.Factory, nil)
			got.Sort(lt)
...
func TestReadDirNames(t *testing.T) {
	...
	expected := []string{"dir.go", "dir_test.go", "doc.go", "exts.go",
		...
		"lfilemock", "match.go", "match_test.go", "multiglob.go",
		"multiglob_test.go", "path.go", "path_test.go", "paths.go",
		"paths_test.go", "repository.go", "search.go", "search_test.go"}
