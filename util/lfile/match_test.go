...
func TestNewMatch(t *testing.T) {
	...
	m = lerr.Must(lfile.RegexMatch(lfile.Exts(false, "txt"), `\/.*dir\/`, `\/\..*/`))
...
func TestMRILMock(t *testing.T) {
	...
	tt := map[string]struct {
		...
		"basic": {
			Match: lerr.Must(lfile.RegexMatch(`\/[a-z]$`, `[0-9]$`, lfile.FilterHidden)),
			expectedFiles: slice.Slice[string]{
		...
		"no-dirs": {
			Match: lerr.Must(lfile.RegexMatch(`\/[a-z]$`, ``, lfile.FilterHidden)),
		...
		"no-files": {
			Match:        lerr.Must(lfile.RegexMatch(``, `[0-9]$`, lfile.FilterHidden)),
			expectedDirs: []string{"/dir/dir2", "/dir/dir1"},
		},
		"no-filter": {
			Match: lerr.Must(lfile.RegexMatch(`\/[a-z]$`, `[0-9]$`, ``)),
			expectedFiles: []string{
...
func TestMRIErr(t *testing.T) {
	...
	mr := lerr.Must(lfile.RegexMatch(`\/[a-z]$`, `[0-9]$`, lfile.FilterHidden)).
		Root("/")
	mr.CoreFS = dir.Repository()
	i, _ := mr.Iterator()
