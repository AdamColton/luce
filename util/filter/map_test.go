...
func TestMapSliceAndMap(t *testing.T) {
	...
	m := lmap.FromIter(mi)
	tform := morph.NewValAll(m.GetVal)
	...
	for n, tc := range tt {
		t.Run(n, func(t *testing.T) {
			...
			expectedVals := slice.FromIter(tform.Iter(tc.keys.Iter()), nil)
...
func TestPurge(t *testing.T) {
	...
	mi := morph.NewValAll(intStrKV).Iter(i)
	...
	expected := lmap.FromIter(morph.NewValAll(intStrKV).Iter(i))
