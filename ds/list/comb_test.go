...
func TestSliceCombinator(t *testing.T) {
	a := list.NewGenerator(5, func(i int) float64 {
		f := float64(i)
		return f + 5
	})
	b := list.NewGenerator(5, func(i int) float64 {
		f := float64(i)
		return f + 15
	})
	got := list.SliceCombinator(ints.Pair, a, b).ToSlice(nil)
