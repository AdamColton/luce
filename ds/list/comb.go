...
// Setter will set part or all of T. For instance T may be a struct and the
// Setter will set a field, or T may be a slice and the setter will set the
// value at some index.
...
// NewListSetter Creates a ListSetter.
...
// InitSetter creates a ListSetter that wraps the setter func, creating a new
// instance of T before invoking the setter function.
...
// NewSetter creates a ListSetter that wraps fn to return the To value.
...
// Chain several Setters together. The power is that the concrete ListSetter
// has parameters of From and To where the Setter interface it fulfills only
// has the To parameter. Generally To will be a struct and each ListSetter
// will set one field.
...
// ListSetter is used to Set one value from a Combinator index.
...
// Combinator combines a Setter with an ints.CombinatorFactory. The
// CombinatorFactory defines the index values for the Setter.
...
// SliceCombinator uses the factory to combine values from the lists.
