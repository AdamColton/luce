...
// Rule to Check
// a red node may only have black children
// all paths have equal black nodes
...
func (n *node[Key, Val]) next(dir int) *node[Key, Val] {
	c := n
	for c.prt != nil && c.prt.Get().getChild(dir) == c {
		nxt := c.prt.Get().getChild(dir)
