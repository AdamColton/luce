...
func (i webSocketInitilizer) Initilize(fieldType reflect.Type) Injector {
	...
	if hasToField {
		lerr.Must(checkSend(toField.Type))
		di.to = toField.Index
	}
	if hasFromField {
		lerr.Must(checkRecv(fromField.Type))
		di.from = fromField.Index
	}
	if hasConnField {
		lerr.Must(checkConn(connField.Type))
...
func (di webSocketDataInserter) Inject(w http.ResponseWriter, r *http.Request, data reflect.Value) (func([]reflect.Value), error) {
	...
	if di.to != nil || di.from != nil {
		...
		if di.from != nil {
			...
			go (iobus.Config{
				CloseOnEOF: true,
			}).Reader(sw, from, nil, &stop)
		}
	}

	callbackClose := func(rets []reflect.Value) {
		conn.Close()
		defer func() {
