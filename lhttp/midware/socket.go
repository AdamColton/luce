...
const (
	// ErrSocketToType is the panic value if the "to" field is not a chan<- []byte.
	ErrSocketToType = lerr.Str("WebSocket ChannelInitilizer 'to' field must be chan<- []byte")

	// ErrSocketFromType is the panic value if the "from" field is not a <-chan []byte.
	ErrSocketFromType = lerr.Str("WebSocket ChannelInitilizer 'from' field must be <-chan []byte")

	// ErrSocketToName is the panic value used when the target field should be
	// *websocket.Conn but is not.
	ErrSocket = lerr.Str("WebSocket Initilizer field must be type *websocket.Conn")
)

var (
	checkSend = filter.
			IsNilRef((*chan<- []byte)(nil)).
			Check(ErrSocketToType)

	checkRecv = filter.
			IsNilRef((*<-chan []byte)(nil)).
			Check(ErrSocketFromType)
