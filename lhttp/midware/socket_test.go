...
func (mf *magicFinalizer) Initilize(t reflect.Type) Injector {
	return mf
}

func (mf *magicFinalizer) Inject(w http.ResponseWriter, r *http.Request, dst reflect.Value) (func([]reflect.Value), error) {
...
func TestWebSocketChannelInitilizer(t *testing.T) {
	ws := NewWebSocket()
	mf := &magicFinalizer{}
	m := New().Inits(ws.Initilizer("To", "From", ""), mf)
...
func TestWebSocketInitilizer(t *testing.T) {
	ws := NewWebSocket()
	m := New().Inits(ws.Initilizer("", "", "Socket"))
